...
jQuery (document).ready (function () {
  ...
  if (getParameterByName ("username").length == 0) {
    ...
    jQuery ('#found-username').empty ().append (getParameterByName ("username"));
  }

  if (getParameterByName ("hashtags").length == 0) {
    jQuery ('#found-hashtags').empty ().append ("none");
  } else {
    var hashtags = getParameterByName ("hashtags")
      .split (",")
  ...
  if (getParameterByName ("filter").length == 0) {
    jQuery ('#found-filter').empty ().append ("filter");
  } else {
    jQuery ('#found-filter').empty ().append (getParameterByName ("filter"));
  }

  // Determine interval.
  /*
  var interval;
  if (getParameterByName ("date_interval") == "lt") {
    interval = "before ";
  } else if (getParameterByName ("date_interval") == "eq") {
    interval = "on ";
  } else {
    interval = "after ";
  }
  ...
  */
  ...
  jQuery.ajax ('/search' + location.search, {
